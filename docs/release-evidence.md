...
| Performance/capacity | Protect app, provider availability, and approved spend during bursts | Static pages, distributed client limiting, provider-wide TPS pacing and daily cost ceiling | Static production build; limiter/budget composition and browser exhaustion-path tests | Load test preview and verify p95, Redis, Amadeus telemetry, and daily reset behavior | No production baseline; upstream searches remain uncached |
