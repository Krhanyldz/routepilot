...
describe("live search browser client", () => {
  ...
  it("normalizes live offers and never accepts demo data", async () => {
    ...
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(json({
      status: "success",
      result: { providerId: "contract-test-provider", fetchedAt: "2026-07-22T12:00:00.000Z", offers: [offer], warnings: ["limited"], coverage: "provider-limited" },
    }));
...
function liveOffer() {
  return {
    id: "offer-1", providerId: "contract-test-provider", dataSource: "live", sourceRecordId: "1", fetchedAt: "2026-07-22T12:00:00.000Z",
